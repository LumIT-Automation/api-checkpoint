...
function System_codeCollect()
{
    ...
    cp -R ../checkpoint $workingFolderPath/var/www/api
    cp ../license.txt $workingFolderPath/var/www/api

    # Remove development db data.
    if [ -f $workingFolderPath/var/www/api/checkpoint/sql/checkpoint.data-development.sql ]; then
        rm -f $workingFolderPath/var/www/api/checkpoint/sql/checkpoint.data-development.sql
    fi

    # Remove __pycache__ folders and not-required ones.
    rm -fR $workingFolderPath/var/www/api/checkpoint/tests
...
function System_codeFilesPermissions()
{
    ...
    chmod +x $workingFolderPath/var/www/api/checkpoint/sql/migrate.sh

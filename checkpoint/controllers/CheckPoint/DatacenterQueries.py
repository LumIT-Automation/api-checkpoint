...
class CheckPointDatacenterQueriesController(CustomControllerCheckPointGetList, CustomControllerCheckPointCreate):
    def __init__(self, *args, **kwargs):
        super().__init__(subject="datacenter_query", *args, **kwargs)
    ...
    def get(self, request: Request, assetId: int, domain: str) -> Response:
        def actionCallback():
            ...
            return DatacenterQuery.listQuick(sessionId="", assetId=assetId, domain=domain, localOnly=localOnly)
    ...
    def post(self, request: Request, assetId: int, domain: str) -> Response:
        return self.create(
            ...
            assetId=assetId,
            domain=domain,
            actionCallback=lambda data: DatacenterQuery.add(sessionId=self.sessionId, assetId=assetId, domain=domain, data=data),
            permission={
                "args": {
                    "assetId": assetId,
                    "domain": domain

...
class CheckPointGroupHostController(CustomControllerBase):
    ...
    def delete(self, request: Request, assetId: int, domain: str, groupUid: str, hostUid: str) -> Response:
        ...
        try:
            if Permission.hasUserPermission(groups=user["groups"], action="group_host_delete", assetId=assetId, domain=domain) or user["authDisabled"]:
                Log.actionLog("Group host removal", user)

                lock = Lock("Group", locals(), groupUid)
                if lock.isUnlocked():
                    lock.lock()

                    GroupHost(sessionId=self.sessionId, assetId=assetId, domain=domain, groupUid=groupUid, hostUid=hostUid).remove()

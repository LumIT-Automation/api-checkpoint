...
class CheckPointNatRuleObjectsController(CustomControllerBase):
    ...
    def get(self, request: Request, assetId: int, domain: str, packageUid: str, natRuleUid: str) -> Response:
        ...
        try:
            if Permission.hasUserPermission(groups=user["groups"], action="rule_objects_get", assetId=assetId) or user["authDisabled"]:
                Log.actionLog("NAT rule objects info: domain: "+str(domain)+" uid: "+str(natRuleUid), user)
                ...
                if lock.isUnlocked():
                    ...
                    data = {
                        "data": {
                            "items": NatRuleObject.listObjectsInNatRule(sessionId="", assetId=assetId, domain=domain, packageUid=packageUid, natRuleUid=natRuleUid)
                        },
    ...
    def post(self, request: Request, assetId: int, domain: str, packageUid: str, natRuleUid: str) -> Response:
        ...
        try:
            if Permission.hasUserPermission(groups=user["groups"], action="rule_objects_post", assetId=assetId) or user["authDisabled"]:
                Log.actionLog("NAT rule objects addition", user)
                ...
                if serializer.is_valid():
                    ...
                    if lock.isUnlocked():
                        ...
                        NatRuleObject.addObjectsToNatRule(sessionId=self.sessionId, assetId=assetId, domain=domain, packageUid=packageUid, natRuleUid=natRuleUid, data=data)

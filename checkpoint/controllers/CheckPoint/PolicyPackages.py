...
class CheckPointPolicyPackagesController(CustomControllerCheckPointGetList, CustomControllerCheckPointCreate):
    def __init__(self, *args, **kwargs):
        super().__init__(subject="policy_package", *args, **kwargs)
    ...
    def get(self, request: Request, assetId: int, domain: str) -> Response:
        def actionCallback():
            ...
            return PolicyPackage.listQuick(sessionId="", assetId=assetId, domain=domain, localOnly=localOnly)
    ...
    def post(self, request: Request, assetId: int, domain: str) -> Response:
        return self.create(
            ...
            assetId=assetId,
            domain=domain,
            actionCallback=lambda data: PolicyPackage.add(sessionId=self.sessionId, assetId=assetId, domain=domain, data=data),
            permission={
                "args": {
                    "assetId": assetId,
                    "domain": domain

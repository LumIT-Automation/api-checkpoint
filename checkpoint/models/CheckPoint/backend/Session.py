                ...
                while True:
                    try:
                        ...
                        if taskStatus == "succeeded":
                            ...
                            raise CustomException(status=400, payload={"CheckPoint": "Assign task failed for domain " + domain})

                        if time.time() >= t0 + timeout: # timeout reached.
                            raise CustomException(status=400, payload={"CheckPoint": "Assign task timeout for domain " + domain})
